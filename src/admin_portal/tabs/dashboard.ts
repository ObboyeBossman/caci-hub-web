...
import { members, groups, broadcasts, auditLogs } from '../store';
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white border border-[#e6edf3] p-5 rounded-2xl shadow-3xs flex items-center justify-between">
          <div>
            ...
            <span class="text-3xl font-black text-amber-600 block mt-1">${auditLogs.length}</span>
          </div>
...
function renderRecentLogs() {
  const reversed = [...auditLogs].reverse().slice(0, 5);
