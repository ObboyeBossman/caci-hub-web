...
import { fmtDate, injectMembershipCSS } from '@modules/membership/utils/member-helpers'
...
<div class="mm-root" style="padding:24px;max-width:1000px;margin:0 auto;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:12px;">
    <div>
      <h2 style="margin:0;font-size: var(--text-2xl);font-weight:700;color:var(--mm-text-primary);">Audit Log</h2>
      <div style="font-size: var(--text-base);color:var(--mm-text-secondary);margin-top:2px;">
  ...
  ${rows.length === 0
    ? `<div style="text-align:center;padding:60px 20px;color:var(--mm-text-secondary);font-size: var(--text-base);">
             ...
             <tr>
               <td>
                 <button class="mm-btn-link" data-al-member="${r.member_id}"
                   style="background:none;border:none;cursor:pointer;color:var(--mm-blue);font-size: var(--text-base);padding:0;text-decoration:underline;">
                   ${r.member_name ?? r.member_id.slice(0, 8) + '…'}
                 </button>
               </td>
               <td style="font-size: var(--text-sm);font-family:monospace;">${r.field_changed}</td>
               <td style="font-size: var(--text-sm);color:var(--mm-text-muted);max-width:180px;word-break:break-all;">${r.old_value ?? '—'}</td>
               <td style="font-size: var(--text-sm);max-width:180px;word-break:break-all;">${r.new_value ?? '—'}</td>
               <td style="font-size: var(--text-sm);">${r.changed_by_name ?? 'System'}</td>
               <td style="font-size: var(--text-sm);white-space:nowrap;">${fmtDate(r.changed_at)}</td>
             </tr>`).join('')}
           </tbody>
         </table>
       </div>`}
