// src/modules/communication/widgets/adminWidgets.ts
// Reusable UI widgets extracted from the Communication module.
...
/* ══════════════════════════════════════════════════════════════════════════
   ADMIN WIDGETS — shared across all communication tabs
   Scoped under: .cw-* prefix
══════════════════════════════════════════════════════════════════════════ */
...
@media (max-width: 900px) { .cw-stats-row { grid-template-columns: repeat(2, 1fr); } }
@media (max-width: 480px) { .cw-stats-row { grid-template-columns: 1fr 1fr; gap: var(--space-sm); } }
...
.cw-stat:active { transform: translateY(0) scale(0.98); }
.cw-stat.active {
  border-width: 1.5px;
...
.cw-stat:hover .cw-stat-hint { opacity: 1; }
.cw-stat.active .cw-stat-hint { opacity: 0; }
.cw-stat-icon-row {
...
.cw-stat-sub { font-size: 11px; font-weight: 400; color: var(--text-secondary); }
.cw-stat-bar {
...
.cw-search i { font-size: 15px; color: var(--text-muted); flex-shrink: 0; transition: color 0.2s; }
.cw-search:focus-within i { color: var(--caci-blue-light); }
.cw-search input {
...
.cw-filter-wrap { position: relative; display: flex; align-items: center; }
.cw-filter-wrap i {
...
.cw-filter-select option { background: var(--bg-card); color: var(--text-primary); }
...
@media (max-width: 860px) {
  ...
  .cw-tbtn { padding: 0 10px; }
  .cw-filter-select {
    min-width: 40px; width: 40px; padding: 0;
    color: transparent; background-image: none;
  }
  .cw-filter-wrap i { left: 50%; transform: translateX(-50%); }
}
@media (max-width: 540px) {
  .cw-search { display: none; }
  .cw-mob-search-btn { display: flex !important; }
  .cw-toolbar.search-open { flex-wrap: wrap; gap: 10px; }
  .cw-toolbar.search-open .cw-search {
    display: flex; width: 100%; max-width: none; order: -1;
    animation: awFadeUp 0.3s cubic-bezier(0.16,1,0.3,1) both;
  }
  .cw-toolbar.search-open .cw-mob-search-btn { display: none !important; }
}
...
.cw-empty-icon-wrap i { font-size: 28px; color: var(--text-muted); }
.cw-empty-title {
...
.cw-ctx-item i { font-size: 16px; color: var(--text-secondary); }
.cw-ctx-item:hover { background: var(--bg-hover); }
.cw-ctx-item:hover i { color: var(--text-primary); }
.cw-ctx-item.danger { color: var(--caci-red); }
.cw-ctx-item.danger i { color: var(--caci-red); }
.cw-ctx-item.danger:hover { background: rgba(198,0,38,0.08); }
.cw-ctx-item.success { color: #22c55e; }
.cw-ctx-item.success i { color: #22c55e; }
.cw-ctx-item.success:hover { background: rgba(34,197,94,0.08); }
.cw-ctx-divider { height: 1px; background: var(--border-default); margin: 3px 0; }
...
.cw-bulk-count i { font-size: 16px; color: var(--caci-blue-light); }
.cw-bulk-spacer { flex: 1; }
...
.cw-col-hd:hover { color: var(--text-secondary); }
.cw-col-hd i { font-size: 12px; }
.cw-col-hd.sorted { color: var(--caci-blue-light); }
.cw-table-body { display: flex; flex-direction: column; }
.cw-table-row {
  display: grid;
  align-items: center;
  min-height: 64px;
...
.cw-table-row:hover { background: rgba(255,255,255,0.025); }
.cw-table-row.selected { background: rgba(0,75,160,0.06); }
.cw-col-cell { padding: 0 8px; display: flex; align-items: center; }

/* Table → mobile card swap */
.cw-mobile-rows { display: none; flex-direction: column; gap: 10px; }
@media (max-width: 639px) {
  .cw-table-header { display: none; }
  .cw-table-body   { display: none; }
  .cw-table-wrap {
...
.cw-results-meta strong { color: var(--text-primary); }
...
.cw-sort-inline:focus { border-color: var(--caci-blue); }
.cw-sort-inline option { background: var(--bg-card); }
...
.cw-filter-banner.show { display: flex; }
...
[data-theme="light"] .cw-filter-pill { color: var(--caci-blue); }
.cw-filter-clear {
...
.cw-filter-clear:hover { border-color: var(--border-strong); color: var(--text-primary); }
...
.cw-chk:hover:not(:checked) { border-color: var(--text-secondary); }
...
.cw-badge-dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }
.cw-badge-active   { background: rgba(34,197,94,0.1);  color: #56d364; }
.cw-badge-active   .cw-badge-dot { background: #22c55e; }
.cw-badge-inactive { background: var(--bg-hover); color: var(--text-secondary); }
.cw-badge-inactive .cw-badge-dot { background: var(--text-muted); }
.cw-badge-pending  { background: rgba(210,153,34,0.1); color: #d29922; }
.cw-badge-pending  .cw-badge-dot { background: #d29922; }
.cw-badge-locked   { background: rgba(198,0,38,0.1);   color: var(--caci-red); }
.cw-badge-locked   .cw-badge-dot { background: var(--caci-red); }
...
.cw-role-pill i { font-size: 12px; color: var(--caci-blue-light); }
...
.cw-toggle-track.on { background: var(--caci-blue); border-color: var(--caci-blue-light); }
.cw-toggle-thumb {
  position: absolute; top: 2px; left: 2px;
  width: 14px; height: 14px; border-radius: 50%; background: var(--text-muted);
  transition: transform 0.2s cubic-bezier(0.16,1,0.3,1), background 0.2s;
}
.cw-toggle-track.on .cw-toggle-thumb { transform: translateX(16px); background: #fff; }
...
.cw-avatar-ring.inactive { background: conic-gradient(var(--text-muted) 0deg 360deg); }
.cw-avatar-ring.pending  { background: conic-gradient(#d29922 0deg 200deg, var(--border-strong) 200deg 360deg); }
.cw-avatar-inner {
...
.cw-modal-header-icon i { font-size: 18px; }
.cw-modal-title { font-size: 15px; font-weight: 600; color: var(--text-primary); margin: 0; }
.cw-modal-subtitle { font-size: 11px; color: var(--text-secondary); margin-top: 1px; }
.cw-modal-close-btn {
...
.cw-modal-close-btn:hover { background: var(--bg-hover); color: var(--text-primary); }
.cw-modal-body {
...
.cw-form-textarea { height: auto; padding: 10px 14px; resize: vertical; min-height: 80px; }
.cw-form-inp::placeholder,
.cw-form-textarea::placeholder { color: var(--text-muted); }
.cw-form-inp:focus,
...
.cw-form-select option { background: var(--bg-card); }
.cw-form-error { font-size: 11px; color: var(--caci-red); margin-top: 3px; display: none; }
...
.cw-scaffold-icon i { font-size: 32px; color: var(--text-muted); }
.cw-scaffold-title {
...
export class StatsCardGroup {
  ...
  constructor(
    ...
    this._el.className = 'cw-stats-row'
  ...
  private render(): void {
    this._el.innerHTML = this._cards.map(c => {
      ...
      const sub = c.getSub?.() ?? ''
      return `
      <div class="cw-stat${this._activeId === c.id ? ' active' : ''}" 
           data-stat-id="${c.id}"
           style="--stat-accent:${c.accentColor};--stat-glow:${c.glowColor};">
        <div class="cw-stat-hint"><i class="bi bi-funnel"></i> Filter</div>
        <div class="cw-stat-icon-row">
          <div class="cw-stat-icon" style="background:${c.glowColor};">
            <i class="bi bi-${c.icon}" style="color:${c.accentColor};font-size:15px;"></i>
          </div>
          <span class="cw-stat-label">${c.label}</span>
        </div>
        <div class="cw-stat-value">${val}${sub ? `<span class="cw-stat-sub">${sub}</span>` : ''}</div>
        <div class="cw-stat-bar">
          <div class="cw-stat-bar-fill" style="width:100%;background:linear-gradient(90deg,${c.accentColor},${c.glowColor});"></div>
        </div>
      </div>`
    }).join('')
...
export class Toolbar {
  ...
  private _render(): void {
    const filters = (this._cfg.filters ?? []).map(f => `
      <div class="cw-filter-wrap">
        ...
        <select class="cw-filter-select" data-filter-id="${f.id}">
          ${f.options.map(o => `<option value="${o.value}">${o.label}</option>`).join('')}
        </select>
      </div>`).join('')

    const actions = (this._cfg.actions ?? []).map(a => `
      <button class="cw-tbtn${a.variant === 'primary' ? ' cw-tbtn-primary' : a.variant === 'danger' ? ' cw-tbtn-danger' : ''}"
              data-action-id="${a.id}" title="${a.label}">
        <i class="bi bi-${a.icon}"></i>
        <span class="cw-btn-label">${a.label}</span>
      </button>`).join('')
    ...
    this._el.innerHTML = `
      ...
      <button class="cw-tbtn cw-mob-search-btn" style="display:none;" title="Search">
    ...
    // Search input
    const inp = this._el.querySelector<HTMLInputElement>('.cw-search input')!
    ...
    // Mob search toggle
    this._el.querySelector('.cw-mob-search-btn')?.addEventListener('click', () => {
  ...
  getSearchValue(): string {
    return this._el.querySelector<HTMLInputElement>('.cw-search input')?.value ?? ''
  }
...
export function renderEmptyState(container: HTMLElement, cfg: EmptyStateConfig): void {
  container.innerHTML = `
    <div class="cw-empty">
      <div class="cw-empty-icon-wrap">
        <i class="bi bi-${cfg.icon}"></i>
      </div>
      <p class="cw-empty-title">${cfg.title}</p>
      <p class="cw-empty-desc">${cfg.description}</p>
      ${cfg.action ? `
      <button class="cw-tbtn cw-tbtn-primary" id="cw-empty-action" style="margin-top:4px;">
        <i class="bi bi-${cfg.action.icon}"></i>
        <span>${cfg.action.label}</span>
      </button>` : ''}
    </div>`
  if (cfg.action) {
    container.querySelector('#cw-empty-action')?.addEventListener('click', cfg.action.onClick)
  }
}
...
export class ContextMenu {
  ...
  show(
    ...
    menu.className = 'cw-ctx'
    menu.style.display = 'block'

    menu.innerHTML = items.map(item => `
      ${item.divider ? '<div class="cw-ctx-divider"></div>' : ''}
      <div class="cw-ctx-item${item.variant ? ' ' + item.variant : ''}" data-item-id="${item.id}">
...
export class BulkActionBar {
  ...
  constructor(container: HTMLElement, actions: BulkBarAction[], onClear: () => void) {
    ...
    this._el = document.createElement('div')
    this._el.className = 'cw-bulk-bar'
    container.appendChild(this._el)
    this._render()
  }
  ...
  private _render(): void {
    ...
    this._el.innerHTML = `
      <span class="cw-bulk-count">
        <i class="bi bi-check2-square"></i>
        ${n} selected
      </span>
      <div class="cw-bulk-spacer"></div>
      ${this._actions.map(a => `
        <button class="cw-tbtn${a.variant === 'danger' ? ' cw-tbtn-danger' : ''}"
                style="height:36px;" data-bulk-id="${a.id}" title="${a.label}">
          <i class="bi bi-${a.icon}"></i>
          <span class="cw-btn-label">${a.label}</span>
        </button>`).join('')}
      <button class="cw-tbtn" style="height:36px;padding:0 10px;" id="cw-bulk-clear" title="Clear selection">
    ...
    this._el.querySelector('#cw-bulk-clear')
...
export function openModal(cfg: ModalConfig): () => void {
  const existingOverlay = document.getElementById('cw-shared-modal')
    ...
    <div class="cw-modal">
      <div class="cw-modal-header">
        <div style="display:flex;align-items:center;gap:12px;">
          <div class="cw-modal-header-icon" style="background:${cfg.iconBg ?? 'rgba(0,75,160,0.12)'};">
            <i class="bi bi-${cfg.icon}" style="color:${cfg.iconColor ?? 'var(--caci-blue-light)'}"></i>
          </div>
          <div>
            <div class="cw-modal-title">${cfg.title}</div>
            ${cfg.subtitle ? `<div class="cw-modal-subtitle">${cfg.subtitle}</div>` : ''}
          </div>
        </div>
        <button class="cw-modal-close-btn" id="cw-modal-close">
          <i class="bi bi-x-lg"></i>
        </button>
      </div>
      <div class="cw-modal-body">${cfg.body}</div>
      <div class="cw-modal-footer">${cfg.footer}</div>
    </div>`
...
export function renderScaffold(
  ...
  container.innerHTML = `
    <div class="cw-scaffold">
      <div class="cw-scaffold-icon">
        <i class="bi bi-${icon}"></i>
      </div>
      <p class="cw-scaffold-title">${title}</p>
      <p class="cw-scaffold-desc">${description}</p>
      <span class="cw-scaffold-badge">
...
export function avatarRingClass(status: string): string {
  if (status === 'inactive' || status === 'locked') return 'cw-avatar-ring inactive'
  if (status === 'pending') return 'cw-avatar-ring pending'
  return 'cw-avatar-ring'
}