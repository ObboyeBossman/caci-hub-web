...
<div class="mm-root" style="padding:24px;max-width:1000px;margin:0 auto;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:12px;">
    <div>
      <h2 style="margin:0;font-size: var(--text-2xl);font-weight:700;color:var(--mm-text-primary);">Reports</h2>
      <div style="font-size: var(--text-base);color:var(--mm-text-secondary);">Membership statistics and insights</div>
    </div>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:24px;">
    ${[['Total Members', total, 'var(--mm-blue)'],['Active', active, 'var(--mm-green)'],['Visitors', visitors, '#0969da'],['New (30d)', newMonth, 'var(--mm-gold)']].map(([label, val, color]) =>
      `<div style="background:var(--mm-bg-card);border:1px solid var(--mm-border);border-radius:12px;padding:18px;">
         <div style="font-size: var(--text-4xl);font-weight:700;color:${color};">${val}</div>
         <div style="font-size: var(--text-sm);color:var(--mm-text-secondary);margin-top:4px;">${label}</div>
       </div>`
    ).join('')}
  </div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:24px;">
    ...
    <div style="background:var(--mm-bg-card);border:1px solid var(--mm-border);border-radius:12px;padding:20px;">
      <div style="font-size: var(--text-base);font-weight:600;color:var(--mm-text-primary);margin-bottom:14px;">By Gender</div>
    ...
    <!-- Status chart -->
    <div style="background:var(--mm-bg-card);border:1px solid var(--mm-border);border-radius:12px;padding:20px;">
      <div style="font-size: var(--text-base);font-weight:600;color:var(--mm-text-primary);margin-bottom:14px;">By Status</div>
  ...
  <!-- Quick report links -->
  <div style="background:var(--mm-bg-card);border:1px solid var(--mm-border);border-radius:12px;padding:20px;">
    <div style="font-size: var(--text-base);font-weight:600;color:var(--mm-text-primary);margin-bottom:14px;">Available Reports</div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:10px;">
      ...
      <div class="mm-report-card" data-rpt-type="${type}"
        ...
        <div style="font-size: var(--text-base);font-weight:600;color:var(--mm-text-primary);" class="mm-report-name">${name}</div>
        <div style="font-size: var(--text-xs);color:var(--mm-text-muted);margin-top:4px;">Click to generate</div>
      </div>`).join('')}
    </div>
  </div>
</div>`
