...
function _openMemberModal(editId: string | null = null): void {
  if (!_state || !_container) return
  _state.editingId = editId

  const modal = _container.querySelector('#mm-memberModal')!
  const overlay = _container.querySelector('#mm-memberModalOverlay')!
  const title = _container.querySelector('#mm-memberModalTitle')!
  const saveBtn = _container.querySelector<HTMLButtonElement>('#mm-memberModalSaveBtn')!

  title.textContent = editId ? 'Edit Member' : 'Add New Member'
  saveBtn.textContent = editId ? 'Save Changes' : 'Register Member'

  // Reset all fields
  _clearMemberForm()

  if (editId) {
    const m = _state.members.find(x => x.id === editId)
    if (m) _populateMemberForm(m)
  } else {
    // Default join date = today
    const today = new Date().toISOString().split('T')[0]
    const jEl = _container.querySelector<HTMLInputElement>('#mm-fJoined')
    if (jEl) jEl.value = today
  }
...
function _closeMemberModal(): void {
  if (!_container) return
  _container.querySelector('#mm-memberModal')?.classList.remove('open')
  _container.querySelector('#mm-memberModalOverlay')?.classList.remove('open')
  document.body.style.overflow = ''
  if (_state) _state.editingId = null
}
...
function _clearMemberForm(): void {
  ...
  ids.forEach(id => {
    const el = _container!.querySelector<HTMLInputElement>(`#${id}`)
    if (el) el.value = ''
  })
  const selects = [
  ...
  _container.querySelector<HTMLInputElement>('#mm-fDOB')!.value = ''
  // Clear errors
  _container.querySelectorAll('.mm-form-error').forEach(e => e.classList.remove('show'))
  _container.querySelectorAll('.mm-form-input,.mm-form-select').forEach(e => e.classList.remove('error'))
}

function _populateMemberForm(m: MemberView): void {
  if (!_container) return
  ...
  set('mm-fJoined', m.join_date ?? '')
  set('mm-fHousehold', m.household_id ?? '')
  set('mm-fECName', m.emergency_contact_name ?? '')
  set('mm-fECPhone', m.emergency_contact_phone ?? '')
  set('mm-fECRel', m.emergency_contact_relationship ?? '')
  set('mm-fNotes', m.pastoral_notes ?? '')
}
...
async function _saveMember(): Promise<void> {
  ...
  const get = (id: string) =>
    (_container!.querySelector<HTMLInputElement | HTMLSelectElement>(`#${id}`)?.value ?? '').trim()
  ...
  const showErr = (errId: string, inputId: string) => {
    _container!.querySelector(`#${errId}`)?.classList.add('show')
  ...
  const payload = {
    ...
    occupation: get('mm-fOccupation') || null,
    physical_address: get('mm-fAddress') || null,
    join_date: get('mm-fJoined') || null,
    household_id: get('mm-fHousehold') || null,
    emergency_contact_name: get('mm-fECName') || null,
    emergency_contact_phone: get('mm-fECPhone') || null,
    emergency_contact_relationship: get('mm-fECRel') || null,
    pastoral_notes: get('mm-fNotes') || null,
  }

  const saveBtn = _container.querySelector<HTMLButtonElement>('#mm-memberModalSaveBtn')!
  saveBtn.disabled = true
  saveBtn.textContent = _state.editingId ? 'Saving…' : 'Registering…'

  try {
    if (_state.editingId) {
      // Validate with update schema
      const parse = UpdateMemberSchema.safeParse({ id: _state.editingId, ...payload })
      if (!parse.success) {
        Toast.error(parse.error.errors[0]?.message ?? 'Validation failed')
        return
      }
      await updateMember(_state.editingId, payload)
    ...
    saveBtn.textContent = _state.editingId ? 'Save Changes' : 'Register Member'
  }
}
...
function _bindAll(): void {
  ...
  _container.querySelector('#mm-memberModalOverlay')?.addEventListener('click', _closeMemberModal)
  _container.querySelector('#mm-memberModalCancel')?.addEventListener('click', _closeMemberModal)
  _container.querySelector('#mm-memberModalSaveBtn')?.addEventListener('click', _saveMember)
...
<!-- ═══ ADD / EDIT MEMBER MODAL (drawer) ═══ -->
<div class="mm-modal-overlay" id="mm-memberModalOverlay"></div>
<div class="mm-modal-drawer" id="mm-memberModal">
  <div class="mm-modal-header">
    <div class="mm-modal-title" id="mm-memberModalTitle">Add New Member</div>
    <button class="mm-modal-close" id="mm-memberModalCancel">
      <svg viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
    </button>
  </div>
  <div class="mm-modal-body">
    ...
    <div class="mm-form-row">
      <div class="mm-form-field">
        ...
        <div class="mm-form-error" id="mm-err-firstName">First name is required</div>
      </div>
      <div class="mm-form-field">
        <label class="mm-form-label">Last Name <span class="req">*</span></label>
        <input type="text" class="mm-form-input" id="mm-fLastName" placeholder="e.g. Mensah">
        <div class="mm-form-error" id="mm-err-lastName">Last name is required</div>
      </div>
    </div>
    <div class="mm-form-row">
      <div class="mm-form-field">
        <label class="mm-form-label">Gender <span class="req">*</span></label>
        <select class="mm-form-select" id="mm-fGender">
          <option value="">Select gender</option>
          <option value="male">Male</option>
          <option value="female">Female</option>
        </select>
        <div class="mm-form-error" id="mm-err-gender">Gender is required</div>
      </div>
      <div class="mm-form-field">
        <label class="mm-form-label">Date of Birth</label>
        <input type="date" class="mm-form-input" id="mm-fDOB">
      </div>
    </div>
    <div class="mm-form-row">
      <div class="mm-form-field">
        <label class="mm-form-label">Marital Status</label>
        <select class="mm-form-select" id="mm-fMarital">
      ...
      <div class="mm-form-field">
        ...
        <input type="text" class="mm-form-input" id="mm-fOccupation" placeholder="e.g. Teacher">
      </div>
    </div>
    ...
    <div class="mm-form-row">
      ...
      <div class="mm-form-field">
        ...
        <input type="email" class="mm-form-input" id="mm-fEmail" placeholder="name@email.com">
      </div>
    </div>
    <div class="mm-form-field">
      <label class="mm-form-label">Residential Address</label>
      <input type="text" class="mm-form-input" id="mm-fAddress" placeholder="e.g. 12 Tema Station Rd, Assakae">
    </div>

    <div class="mm-form-section-title">Church Information</div>
    <div class="mm-form-row">
      <div class="mm-form-field">
        <label class="mm-form-label">Status <span class="req">*</span></label>
        <select class="mm-form-select" id="mm-fStatus">
      ...
      <div class="mm-form-field">
        <label class="mm-form-label">Date Joined</label>
        <input type="date" class="mm-form-input" id="mm-fJoined">
      </div>
    </div>
    <div class="mm-form-field">
      <label class="mm-form-label">Household</label>
      <select class="mm-form-select" id="mm-fHousehold">
        <option value="">None</option>
      </select>
    </div>

    <div class="mm-form-section-title">Emergency Contact</div>
    <div class="mm-form-row">
      <div class="mm-form-field">
        <label class="mm-form-label">Contact Name</label>
        <input type="text" class="mm-form-input" id="mm-fECName" placeholder="e.g. Ama Mensah">
      </div>
      <div class="mm-form-field">
        <label class="mm-form-label">Contact Phone</label>
        <input type="tel" class="mm-form-input" id="mm-fECPhone" placeholder="+233 24 000 0000">
      </div>
    </div>
    <div class="mm-form-field">
      <label class="mm-form-label">Relationship</label>
      <select class="mm-form-select" id="mm-fECRel">
        <option value="">Select</option>
    ...
    <div class="mm-form-section-title">Notes (Admin Only)</div>
    <div class="mm-form-field">
      <textarea class="mm-form-textarea" id="mm-fNotes" placeholder="Internal admin / pastoral notes about this member…"></textarea>
    </div>
  </div>
  <div class="mm-modal-footer">
    <button class="mm-btn-outline" id="mm-memberModalCancel2" onclick="">Cancel</button>
    <button class="mm-btn-primary" id="mm-memberModalSaveBtn">Register Member</button>
  </div>
</div>
