// src/modules/accounts/repository.ts
// Data access for the accounts module.
// Queries: user_profiles (primary), members (one-off read for name/email when provisioning).
// Does NOT import from membership/ — cross-module isolation is enforced.
...
function mapError(err: unknown, ctx: string): AccountsError {
  if (err instanceof AccountsError) return err
  const msg = (err as any)?.message ?? String(err)
  console.error(`[accounts/${ctx}]`, err)
  return new AccountsError(msg, err)
}
...
export async function updateUserRole(userId: string, role: UserRoleEnum): Promise<void> {
  try {
...
//       } catch {
//         throw error   // only catch JSON parse failure
//       }
...
//   } catch (err) {
//     throw mapError(err, 'provisionAccount')
//   }
// }
...
export async function listAssemblyRoles(assemblyId?: string): Promise<AssemblyRole[]> {
  ...
  try {
    let query = supabase
      .from('assembly_roles')
      .select('id, assembly_id, name, description, role_permissions(permission_id)')
...
export async function createAssemblyRole(payload: {
  ...
  if (!aid) throw new AccountsError('No active assembly — cannot create role')
  try {
    const { data, error } = await supabase
      .from('assembly_roles')
 ...
 * sets user_profiles.role_id → NULL for affected users.
 */
 ...
 * The JWT sync trigger fires automatically on UPDATE of role_id.
