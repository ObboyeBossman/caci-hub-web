...
import { getMemberStub, provisionAccount } from '../repository'
...
const ProvisionAccount: PageModule = {
  async render(container) {
    ...
    if (member?.authUserId) {
      navigate(`/accounts/${member.authUserId}`)
      return
    }
...
<div class="mm-root" style="padding:24px;max-width:540px;margin:0 auto;">
  ...
  <div style="background:var(--mm-bg-card);border:1px solid var(--mm-border);
    ...
    <p style="margin:0 0 24px;font-size:13px;color:var(--mm-text-secondary);">
    ...
    <form id="pa-form">
      ...
      <div style="margin-bottom:16px;">
        ...
        <p style="margin:4px 0 0;font-size:11px;color:var(--mm-text-muted);">
...
`
    ...
    // Back / Cancel
    const goBack = () => history.length > 1 ? history.back() : navigate('/accounts')
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        const { userId } = await provisionAccount({ memberId, role, path: method, email })
